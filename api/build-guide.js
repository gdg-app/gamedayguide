export default async function handler(req, res) {
  try {
    ...
    const radiusMiles = normalizeRadius(radiusMilesRaw);
...
/**
 * Category format:
 * [Label, PlacesType, Keyword, RequiredTypes[], ExcludedTypes[]]
 */
function categories() {
  return [
    ["🍔 Food", "restaurant", "", [], []],
  ...
  return [
    {
      ...
      queryText: "urgent care",
      specialType: "urgent",
      url: buildTextSearchUrl(loc, radiusMeters, "urgent care", apiKey)
    },
    {
      category: "🏥 Urgent Care / ER",
      queryText: "pediatric urgent care",
      specialType: "urgent",
      url: buildTextSearchUrl(loc, radiusMeters, "pediatric urgent care", apiKey)
    },
    {
      category: "🏥 Urgent Care / ER",
      queryText: "emergency room",
      specialType: "urgent",
      url: buildTextSearchUrl(loc, radiusMeters, "emergency room", apiKey)
    }
  ];
}
  ...
  return [
    {
      ...
      queryText: "emergency dentist",
      specialType: "dentist",
      url: buildTextSearchUrl(loc, radiusMeters, "emergency dentist", apiKey)
    },
    {
      category: "🦷 Emergency Dentist",
      queryText: "urgent dental care",
      specialType: "dentist",
      url: buildTextSearchUrl(loc, radiusMeters, "urgent dental care", apiKey)
    },
    {
      category: "🦷 Emergency Dentist",
      queryText: "24 hour dentist",
      specialType: "dentist",
      url: buildTextSearchUrl(loc, radiusMeters, "24 hour dentist", apiKey)
    },
    {
      category: "🦷 Emergency Dentist",
      queryText: "dental emergency",
      specialType: "dentist",
      url: buildTextSearchUrl(loc, radiusMeters, "dental emergency", apiKey)
    }
  ];
}
...
async function collectAllRows(loc, radiusMiles, apiKey) {
  ...
  const responses = await Promise.all(
    allRequests.map(async function (request) {
      try {
        ...
        return { request: request, res: res, error: null };
      } catch (error) {
        return {
          request: request,
          res: null,
  ...
  for (let i = 0; i < responses.length; i++) {
    const request = responses[i].request;
    const res = responses[i].res;
