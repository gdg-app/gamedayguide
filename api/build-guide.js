...
function textMatchesCoffee(p) {
  ...
  const excludeWords = [
    ...
    'nightclub', 'yoga', 'martial arts', 'spa', 'salon', 'restaurant'
  ];
...
function textMatchesFood(p) {
  ...
  if (types.indexOf('restaurant') !== -1 || types.indexOf('meal_takeaway') !== -1 || types.indexOf('meal_delivery') !== -1) {
    return true;
  }
...
function buildStandardCategoryRequests(loc, radiusMeters, apiKey) {
  ...
  for (let i = 0; i < cats.length; i++) {
    const c = cats[i];
    if (isUrgentCareCategory(c[0]) || isSportingGoodsCategory(c[0]) || isGroceryCategory(c[0]) || isEmergencyDentistCategory(c[0]) || isFoodCategory(c[0])) {
      continue;
    }
...
function getFoodRankingScore(p, loc) {
  ...
  if (p.geometry && p.geometry.location && typeof p.geometry.location.lat === 'number' && typeof p.geometry.location.lng === 'number') {
...
function buildRowFromPlace(category, p, loc, radiusMiles) {
  ...
  const openNow = (p.opening_hours && typeof p.opening_hours.open_now === 'boolean')
...
function parseStandardCategoryResponse(request, res, loc, radiusMiles, limit) {
  if (res.status !== 'OK' && res.status !== 'ZERO_RESULTS') {
    throw new Error('Places failed for ' + request.category + ': ' + res.status + (res.error_message ? ' - ' + res.error_message : ''));
  }
...
function collectPlacesFromTextResponse(res, labelForErrors) {
  if (res.status !== 'OK' && res.status !== 'ZERO_RESULTS') {
    throw new Error('Text Search failed for ' + labelForErrors + ': ' + res.status + (res.error_message ? ' - ' + res.error_message : ''));
  }
  return res.results || [];
}
...
function collectPlacesFromNearbyResponse(res, labelForErrors) {
  if (res.status !== 'OK' && res.status !== 'ZERO_RESULTS') {
    throw new Error('Nearby Search failed for ' + labelForErrors + ': ' + res.status + (res.error_message ? ' - ' + res.error_message : ''));
  }
  return res.results || [];
}
...
async function collectAllRows(loc, radiusMiles, apiKey) {
  ...
  const responses = await Promise.all(
    allRequests.map(async function (request) {
      try {
        ...
        return { request, res: null, error: err && err.message ? err.message : String(err) };
      }
    })
  );
  ...
  for (let i = 0; i < responses.length; i++) {
    const { request, res, error } = responses[i];
    ...
    try {
      ...
      // skip failed category without breaking whole guide
    }
  }
  ...
  allRows = allRows.concat(urgentRows).concat(dentistRows).concat(sportingRows).concat(groceryRows).concat(foodRows);
